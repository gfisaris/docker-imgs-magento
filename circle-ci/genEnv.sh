...
if [ "$DEPLOYMENT_ENV" = "prod" ]; then
   MGT_DB_ADMINPASS="$CI_MGT_DB_ADMINPASS_ENV_PROD"
else
   MGT_DB_ADMINPASS="$CI_MGT_DB_ADMINPASS_ENV_INT"
fi
...
MGT_DATE="Fri, 01 Jan 2016 00:00:01 +0000"
MGT_CRKEY="$CI_MGT_CRKEY"
MGT_ADMIN_URL="$CI_MGT_ADMIN_URL"
MGT_SESSION_STORAGE="$CI_MGT_SESSION_STORAGE"
